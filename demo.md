...
## Candidate families from sequences
...
## a partitioning method: k-means estimates no scatter, and the closures
## the constructor generates build the cluster description from the
## returned partition, through clust2params()
KMNS <- mset_kmeans(K = KK)
```

Note that k-means returns labels and centers, no scatter matrices, and yet it is scorable: a hard partition determines a cluster description, `clust2params()` computes it, and the candidate enters the comparison on the same footing as a fitted mixture.
That mechanism is what the next section generalizes.


## Set methods that the package does not provide

`mset_user()` is the general constructor of which all the others are specializations.
It takes the *name* of a prototype function and expands the arguments passed through `...` into one candidate per combination, exactly as above.

The prototype must take `data` as its first argument and must return a list carrying a `params` component with `proportion` (length K), `mean` (P by K), and `cov` (P by P by K).
There are two cases, and it is worth seeing both.



### A method that returns labels only

Hierarchical clustering returns a tree; cutting it returns labels and nothing else.
`clust2params()` supplies the rest. First we program a function wrapper that outputs the desired objects (see the `res` output below).
...
## one candidate per (K, agglomeration method) pair. 'ward.D2' is Ward's
## criterion applied to Euclidean distances
...
`method_name` creates the codename by which a candidate is addressed and printed in the outputs. `.export` names the objects needed by the parallel workers.




### A method that already carries a (size, mean, cov) description

`mclust` fits Gaussian mixtures, so the parameters are in the fitted object already and only need to be renamed into the layout the score expects. The wrapping and the construction of the method set here are much simpler.
...
## one candidate per (K, covariance model) pair; see
## help("mclustModelNames"). '.packages' is the mechanism for a prototype
## whose dependencies are not visible on a parallel worker, whose
## environment does not inherit the current one. Here it also has to
## *attach* mclust rather than merely load its namespace, because Mclust
## resolves some of its own internals by bare name
...
Nothing so far has anything to do with validation. A method set is a list of ready-to-run clustering setups, and it can be inspected and executed as such.
...
``` r
## only the cluster description, which is the form the scoring engine
## consumes
...
`apply_method()` does the same through the set rather than through the element, and returns the fit with a rendered description of the method attached, so that it can say later what it is.
...
A hundred and fifty candidates from five families, stated in five lines.
...
``` r
## the lower percentile bound, at a tail probability other than the
## stored one
...
``` r
## the parsimonious rule. The summaries gain a logical 'selected' column
## flagging the candidates inside the band, and the print method can be
...
Two limits, both worth meeting on purpose rather than in the middle of an analysis.

A `prob` override recomputes the bounds from the raw per-split scores, so it needs them; a `delta` override reads only the stored summary and works on anything.
