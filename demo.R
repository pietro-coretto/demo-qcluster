...
##
## Note that k-means returns labels and centers, no scatter matrices, and yet
## it is scorable: a hard partition determines a cluster description,
## `clust2params()` computes it, and the candidate enters the comparison on
## the same footing as a fitted mixture.
...
## ----------------------------------------------------------------------------
##
## `mset_user()` is the general constructor of which all the others are
## specializations.
## It takes the *name* of a prototype function and expands the arguments
## passed through `...` into one candidate per combination, exactly as above.
##
## The prototype must take `data` as its first argument and must return a list
## carrying a `params` component with `proportion` (length K), `mean` (P by
## K), and `cov` (P by P by K).
...
## nothing else.
## `clust2params()` supplies the rest. First we program a function wrapper
## that outputs the desired objects (see the `res` output below).
##
...
##
## `method_name` creates the codename by which a candidate is addressed and
## printed in the outputs. `.export` names the objects needed by the parallel
## workers.
##
## ----------------------------------------------------------------------------
...
## already and only need to be renamed into the layout the score expects. The
## wrapping and the construction of the method set here are much simpler.
##
library(mclust)
...
## ----------------------------------------------------------------------------
##
## Nothing so far has anything to do with validation. A method set is a list
## of ready-to-run clustering setups, and it can be inspected and executed as
## such.
##
...
##
## `apply_method()` does the same through the set rather than through the
## element, and returns the fit with a rendered description of the method
## attached, so that it can say later what it is.
##
...
## ----------------------------------------------------------------------------
##
## Two limits, both worth meeting on purpose rather than in the middle of an
## analysis.
##
## A `prob` override recomputes the bounds from the raw per-split scores, so
## it needs them; a `delta` override reads only the stored summary and works
## on anything.
##
